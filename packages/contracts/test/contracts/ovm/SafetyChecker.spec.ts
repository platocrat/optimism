...
describe('Safety Checker', () => {
  ...
  describe.only('isBytecodeSafe()', async () => {
    for (const [name, json] of Object.entries(SYNTHETIX_BYTECODE)) {
      it(`${name}: gas cost for init code safety check`, async () => {
        ...
        console.log(`${name}: initcode is ${json.bytecode.length/2} bytes long`)
        ...
        const isSafe: boolean = await safetyChecker.isBytecodeSafe(json.bytecode)
      ...
      it(`${name}: gas cost for deployed bytecode safety check`, async () => {
        ...
        console.log(`${name}: deployed bytecode is ${json.bytecode.length/2} bytes long`)

        const res = await safetyChecker.provider.estimateGas(tx)
        console.log(`${name}: estimate gas result for deployed bytecode: ${res}`)

        const isSafe: boolean = await safetyChecker.isBytecodeSafe(json.deployedBytecode)
        console.log(`${name}: and is it safe? ${isSafe}`)
      })
    }
    // describe('Empty case', () => {
